...
- 支持人机协作与中断恢复
- 可视化执行流与调试支持
- 当系统缺少现成能力时，可进入自主实现模式，通过代码生成补齐 Workflow、Blueprint 或 Feature 缺口

## 1.1 缺失能力的自主实现

LangGraph Workflow / Agent 不应被限制为只能调用现成节点。
当 AI Core 判断当前需求缺少可执行节点时，可以新增一条“实现型路径”：

- 生成缺失 Blueprint 或节点代码
- 生成必要测试与回归校验
- 完成注册后重新进入正常 Workflow / Agent 执行链路

这样系统具备的不是一次性回答，而是“发现缺口 -> 自写实现 -> 回到可复用运行态”的闭环能力。
