...
def test_schedule_information_agent_creation_and_direct_record_flow(isolated_generated_artifacts) -> None:
    ...
    assert "爸爸" in str(first_payload["knowledge"])
    assert "大阪" in str(first_payload["knowledge"])
    ...
    assert "朱棣" in str(second_payload["knowledge"])
    assert "远足" in str(second_payload["knowledge"])
