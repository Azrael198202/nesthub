...
def test_agent_framework_service_generates_information_profile_from_input() -> None:
    ...
    profile = service.resolve_information_profile_definition(
        purpose="主要记录家庭成员的信息",
        signals={
            "combined_text": "主要记录家庭成员的信息",
            "profile_seed": "entity_directory",
            "entity_label": "家庭成员",
            "role_name": "家庭成员信息管理智能体",
            "knowledge_added_message": "已完成添加，并已记录该家庭成员信息。",
    ...
    assert profile["entity_label"] == "家庭成员"
...
def test_information_profile_signal_analyzer_prefers_model_output() -> None:
    class ModelRouterStub:
        async def invoke(self, task_type: str, prompt: str, system_prompt: str | None = None, **kwargs) -> str:
            ...
            return json.dumps(
                {
                    "profile_seed": "structured_timeline",
                    "entity_label": "日程安排",
                    "role_name": "日程信息智能体",
                    "knowledge_added_message": "已经将此信息记录到日程信息智能体。",
                    "query_aliases": {"地点": "location"},
                    "knowledge_schema": [{"key": "actor", "prompt": "这条安排对应的是谁或哪个对象？"}],
                },
                ensure_ascii=False,
            )
    ...
    signals = analyzer.analyze("主要记录出行安排", "需要地点和时间")

    assert signals["profile_seed"] == "structured_timeline"
    assert signals["entity_label"] == "日程安排"
    assert signals["knowledge_schema"][0]["key"] == "actor"
...
def test_information_profile_signal_analyzer_falls_back_when_model_unavailable() -> None:
    ...
    signals = analyzer.analyze("主要记录家庭成员的信息", "")

    assert signals["profile_seed"] == "entity_directory"
    assert signals["entity_label"] == "家庭成员"
