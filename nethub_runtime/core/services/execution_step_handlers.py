...
def _extract_first_date_hint(text: str) -> str:
    match = re.search(r"\d{4}-\d{1,2}-\d{1,2}|\d{1,2}/\d{1,2}|\d{1,2}月\d{1,2}[日号]?", text)
    return str(match.group(0)) if match else ""


def _extract_time_hints(text: str) -> list[str]:
    return [str(item) for item in re.findall(r"\d{1,2}:\d{2}|\d{1,2}点(?:\d{1,2})?", text)]


def _agent_precondition_status(coordinator: Any, context: CoreContextSchema) -> dict[str, bool]:
    checks = {
        "schedule_agent_exists": False,
        "reminder_agent_exists": False,
    }

    # 1) Check current configured agent in session state.
    session_payload = coordinator.session_store.get(context.session_id)
    configured = dict(session_payload.get("configured_agent") or {})
    text_blob = " ".join(
        str(configured.get(key) or "")
        for key in ("name", "role", "description", "knowledge_entity_label", "profile")
    ).lower()
    if any(token in text_blob for token in ("schedule", "calendar", "日程", "行程", "预约")):
        checks["schedule_agent_exists"] = True
    if any(token in text_blob for token in ("reminder", "提醒", "闹钟")):
        checks["reminder_agent_exists"] = True

    # 2) Check promoted agent definitions in vector store.
    if getattr(coordinator, "vector_store", None) is not None:
        try:
            hits = coordinator.vector_store.search("information agent schedule reminder 日程 提醒", top_k=20, namespace="information_agent_fact")
        except Exception:
            hits = []
        for item in hits:
            metadata = dict(item.get("metadata") or {})
            if str(metadata.get("type") or "") != "information_agent_definition":
                continue
            blob = " ".join(
                [
                    str(metadata.get("entity_label") or ""),
                    str(metadata.get("profile") or ""),
                    str(metadata.get("agent_id") or ""),
                ]
            ).lower()
            if any(token in blob for token in ("schedule", "calendar", "日程", "行程", "预约")):
                checks["schedule_agent_exists"] = True
            if any(token in blob for token in ("reminder", "提醒", "闹钟")):
                checks["reminder_agent_exists"] = True
    return checks


def handle_schedule_record_source_check_step(
    coordinator: Any,
    _step: dict[str, Any],
    _task: TaskSchema,
    context: CoreContextSchema,
    ...
    session_payload = coordinator.session_store.get(context.session_id)
    records = list(session_payload.get("records", []))
    sources: list[str] = []
    if records:
        sources.append("session_store")

    promoted_fact_hits: list[dict[str, Any]] = []
    if getattr(coordinator, "vector_store", None) is not None:
        try:
            promoted_fact_hits = coordinator.vector_store.search(
                "schedule reminder appointment",
                top_k=8,
                namespace="information_agent_fact",
            )
        except Exception:
            promoted_fact_hits = []
    if promoted_fact_hits:
        sources.append("information_agent_fact")

    preconditions = _agent_precondition_status(coordinator, context)
    if not sources:
        sources.append("none")
    if preconditions["schedule_agent_exists"]:
        sources.append("schedule_agent")
    if preconditions["reminder_agent_exists"]:
        sources.append("reminder_agent")
    sources = list(dict.fromkeys(sources))
    return {
        "status": "completed",
        "sources": sources,
        "records_count": len(records),
        "knowledge_hits": len(promoted_fact_hits),
        "preconditions": preconditions,
        "preconditions_satisfied": all(preconditions.values()),
        "message": f"schedule sources={','.join(sources)} records={len(records)} knowledge_hits={len(promoted_fact_hits)}",
    }


def handle_schedule_availability_query_step(
    coordinator: Any,
    _step: dict[str, Any],
    task: TaskSchema,
    context: CoreContextSchema,
    ...
    source_check = step_outputs.get("schedule_record_source_check", {}) or {}
    preconditions = dict(source_check.get("preconditions") or {})
    preconditions_satisfied = bool(source_check.get("preconditions_satisfied", True))
    target_date = _extract_first_date_hint(task.input_text) or "target_day"
    session_payload = coordinator.session_store.get(context.session_id)
    records = list(session_payload.get("records", []))
    lowered_target = target_date.lower()
    ...
    for item in records:
        ...
        blob = " ".join(
            ...
            for field in ("time", "content", "message", "title", "text", "location")
        ).lower()
        if lowered_target != "target_day" and lowered_target in blob:
            matched.append(item)
    ...
    return {
        ...
        "reason": "no_appointment_found" if available else "appointment_exists",
        "preconditions": preconditions,
        "preconditions_satisfied": preconditions_satisfied,
        "message": (
            f"{target_date} {'无预约' if available else '已有预约'}"
            if preconditions_satisfied
            else f"{target_date} {'无预约' if available else '已有预约'}（缺少日程/提醒智能体，结果仅基于会话记录）"
        ),
    }


def handle_travel_itinerary_generation_step(
    coordinator: Any,
    _step: dict[str, Any],
    task: TaskSchema,
    _context: CoreContextSchema,
    ...
    availability = step_outputs.get("schedule_availability_query", {}) or {}
    if availability.get("available") is False:
        return {
            "status": "completed",
            "itinerary": "",
            "model_source": "none",
            "summary": "目标日期已有预约，跳过观光计划生成。",
            "message": "已有预约，未生成大阪计划。",
        }

    prompt = (
        "请生成一个大阪一日游计划，输出简洁中文，分时段，包含地点和交通建议。\n"
        f"用户请求: {task.input_text}"
    )
    itinerary = ""
    model_source = "local_fallback"
    if hasattr(coordinator, "_invoke_model_text"):
        generated = coordinator._invoke_model_text(
            task_type="task_planning",
            prompt=prompt,
            system_prompt="你是旅行规划助手。输出纯文本，不要markdown。",
            temperature=0.3,
        )
        if generated:
            itinerary = generated.strip()
            selected_model = ""
            try:
                if getattr(coordinator, "model_router", None) is not None:
                    selected_model = str(coordinator.model_router.select_model("task_planning") or "")
            except Exception:
                selected_model = ""
            if ":" in selected_model:
                provider = selected_model.split(":", 1)[0].lower()
                model_source = "local_model" if provider == "ollama" else "external_model"
            else:
                model_source = "model_router"
    if not itinerary:
        itinerary = "9:00-10:30 大阪城；11:00-13:00 道顿堀；14:00-16:00 梅田空中庭园；16:30-18:00 心斋桥。"
    return {
        "status": "completed",
        "itinerary": itinerary,
        "model_source": model_source,
        "summary": "大阪一日游计划已生成。",
        "message": f"大阪计划已生成（{model_source}）。",
    }


def handle_reminder_create_trigger_step(
    coordinator: Any,
    _step: dict[str, Any],
    task: TaskSchema,
    context: CoreContextSchema,
    ...
    source_check = step_outputs.get("schedule_record_source_check", {}) or {}
    preconditions = dict(source_check.get("preconditions") or {})
    if preconditions and not bool(preconditions.get("reminder_agent_exists")):
        return {
            "status": "blocked",
            "saved": 0,
            "preconditions": preconditions,
            "preconditions_satisfied": False,
            "message": "未检测到提醒智能体，跳过提醒写入。",
        }

    times = _extract_time_hints(task.input_text)
    remind_time = times[0] if times else "18:00"
    flight_time = times[-1] if times else "18:30"
    itinerary = (step_outputs.get("travel_itinerary_generation") or {}).get("itinerary", "")
    reminder_record = {
        "type": "reminder",
        "time": remind_time,
        "content": f"去机场，乘坐{flight_time}航班",
        "source": "schedule_reminder_flow",
        "itinerary_summary": itinerary[:180],
    }
    updated = coordinator.session_store.append_records(context.session_id, [reminder_record])
    return {
        "status": "completed",
        "reminder": reminder_record["content"],
        "saved": 1,
        "total_records": len(updated.get("records", [])),
        "message": f"已创建提醒：{remind_time} 去机场，{flight_time} 航班。",
    }
